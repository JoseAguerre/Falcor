...
Combining light sampling with BRDF sampling via multiple importance sampling requires the
...
\WHILE{$\mathit{queue} \neq \emptyset$ and $|\mathit{leaves}| + |\mathit{queue}| + 3 \leq B$}
  ...
  \FOR{each of the 4 children $C$ of $R$}
    \IF{$C$ is a single texel or $\mathrm{Var}(C) = 0$}
  ...
  \ENDFOR
\ENDWHILE
\STATE $\mathit{leaves} \leftarrow \mathit{leaves} \cup \mathit{queue}$ \COMMENT{whatever's left becomes leaves as-is}
\end{algorithmic}
\end{algorithm}

\subsection{Implementation}
\label{sec:impl}
We implement both variants in Falcor. Each leaf is packed into a 16-byte record -- its
rectangle as two packed 16-bit corner coordinates, the alias threshold and redirect index
as a half-float and a 16-bit index, and $\bar{L}_i$ as a half-float -- and the tree topology
into a flat array of 16-bit nodes, one bit of which flags a node as a leaf. For
BudgetLeafAlias, we size $B$ so the combined leaf and node buffers fit within roughly half
the GPU's per-core L1 data cache, keeping the whole sampling structure resident there during
traversal instead of round-tripping to global memory on every sample. This compactness --
not any change in per-sample statistical efficiency -- is where the technique's real-time
advantage over a dense CDF or a full-resolution alias table comes from.
