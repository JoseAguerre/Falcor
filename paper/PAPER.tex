...
Some previous works also address video-driven lighting. Havran et al.~\cite{havran2005interactive} and Iorns and Rhee~\cite{iorns2015realtime} both light
scenes from captured video environment maps, but neither targets real-time per-frame
resampling of the video itself as the importance distribution. Lu et al.~\cite{lu2013realtime} rebuild a tabulated per-cube-face CDF in real-time for sampling of a dynamic environment map, noting it as an expensive part of the process.  
...
Regarding the use of tree structures for sampling, 
Ergun et al.~\cite{ergun2012realtime} split an environment map into a kd-tree via the greedy partition strategy, to a predefined block count, then fit the sorted leaf probabilities with a closed-form curve for direct inversion. They report concrete gains in memory compression, higher speed and accuracy than CDF inversion at matched performance. Lambers~\cite{lambers2022parameterization} proposes a fixed-grid, parameterization-independent sampler but reports no comparable memory or speed numbers. 
